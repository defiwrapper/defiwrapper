...
export function getUnderlyingTokenPercent(
  ...
  if (version == "V1") {
    ...
    let decimals = BigInt.fromString("10").pow(18).toString();
    if (shareResult)
    ...
    let decimals = BigInt.fromString("10").pow(token.decimals).toString();
    if (shareResult)
      return Big.of(shareResult).div(Big.of(decimals)).toString();
  }
  return "";
...
export function getComponents(
  ...
  let underlyingToken: Token = getUnderlyingToken(token.address, connection);
  if (underlyingToken.address == "Unknown") return [];

  let rate: string = getUnderlyingTokenPercent(token, version, connection);
  if (rate == "") return [];
