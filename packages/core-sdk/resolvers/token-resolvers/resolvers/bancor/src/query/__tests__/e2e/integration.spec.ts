...
describe("Bancor Token Resolver", () => {
  ...
  const BNT_LINK_V1 = "0x7b5b7c0534cd37fd7B637b46D9e9CdD3D7e3acD9";
  ...
  describe("isValidTokenProtocol", () => {
    ...
    test("bancor_v1 BNT-LINK pool", async () => {
      const result = await isValidProtocolToken(BNT_LINK_V1, "bancor_v1", protocolEnsUri, client);
      expect(result.error).toBeFalsy();
      expect(result.data).not.toBeUndefined();
      expect(result.data).toBe(true);
    });
